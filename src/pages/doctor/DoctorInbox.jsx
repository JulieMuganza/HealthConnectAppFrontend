...
import '../../pages/patient/ChatLayout.css';
...
const DoctorInbox = () => {
    ...
    useEffect(() => {
        ...
        if (targetUserId && chats.length > 0) {
            const chat = chats.find(c => c.participant.id === parseInt(targetUserId));
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            // Append message locally (optimistic or from response)
            const sentMsg = {
                id: response.data.id,
                senderId: user.id, // Current user is sender
